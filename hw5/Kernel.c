...
--    Init()
--         Each mutex must be initialized.
--    GiveTo(th: ptr to Thread)
--         Passes the mutex to the next thread on the wait list
--    IsHeldByCurrentThread()
--         Return TRUE iff the current (invoking) thread holds a lock
--         on the mutex.
  ...
  behavior HoareMutex
    ...
    method GiveTo(nextTh: ptr to Thread)
...
-----------------------------  Condition  ---------------------------------
  ...
  -- This is an abstract data type, extending functionality of the Condition Variables with "Hoare" semantics
  -- BLITZ by itself uses a Mesa-style semantics but we are overriding this simulate a hoare style.
  -- This behavior defines three functions: Init(), Wait(), Signal()
  -- We are not showing the Broadcast() as the hoare part happens in Signal()
  -- Init() Initilizes the wait list, Wait() takes in an argument of the monitor''s mutex
  -- Signal() also takes monitor''s mutex as an argument, but while waking the thread up from the wait list, it 
  -- gives the thread the lock directly(right there). 


  ---------------------------  HoareCondition . Init  ------------------------
    --
    -- Initializes wait list
    --
    method Init()
  ...
  ---------------------------  HoareCondition . Signal  ------------------------
    ...
    -- This function signals the next thread in wait list and hands off the mutex to it. Rest of the functionality remains 
    -- same as the Condition.Signal()
    ...
    method Signal(mutex: ptr to HoareMutex)
      ...
      if t
        ...
        ----- Handing off the mutex to the thread that is woken up --------

        mutex.GiveTo(t)
        
        ----- The above line is the change for Hoare semantic --------
      endIf
      oldIntStat = SetInterruptsTo (oldIntStat)
    endMethod
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- Setting all threads initially to UNUSED
        -- Initilize mutex and condition first
          var i: int
          print ("Initializing Thread Manager...\n")

          -- Mutex lock Initialize -- 
          threadManagerMutex = new Mutex
          threadManagerMutex.Init()

          -- Condition Initialize -- 
          threadBecameFree = new Condition
          threadBecameFree.Init()

          -- Init all threads from thread table to free list -- 
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- acquire and release locks as this is an entry point
          var 
            t: ptr to Thread
          ...
          if freeList.IsEmpty()
            while !t
              -- Loop till t != null
              -- pop one out of the freeList
              t = freeList.Remove()
              if !t
                -- if its still empty, make it wait on condition
                threadBecameFree.Wait(&threadManagerMutex)
              endIf
            endWhile
          else
            -- if freeList is not empty, pop one directly. 
            t = freeList.Remove()
          endIf
          -- change status to JUST_CREATED
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- to the FREE list.
        -- acquire and release mutex lock as its an entry point
        threadManagerMutex.Lock()

        -- change the status of th back to UNUSED
        th.status = UNUSED
        freeList.AddToEnd(th)
        -- Signal that free threads are available
        threadBecameFree.Signal(&threadManagerMutex)
  ...
  behavior HoareThreadManager
      ...
      method Init ()
        ...
        -- Initilize mutex and condition first
          ...
          -- Mutex lock Initialize -- 
          threadManagerMutex = new HoareMutex
          threadManagerMutex.Init()
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        -- making this function a "thread-safe" style function
        -- This method returns a new ProcessControlBlock; it will wait
        -- until one is available.
        -- acquire and release locks - monitor approach
          var 
            tempPcb: ptr to ProcessControlBlock
          processManagerLock.Lock()

          if freeList.IsEmpty()
            while !tempPcb
              tempPcb = freeList.Remove()
              if !tempPcb
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
          ...
          var i, frameAddr, frame: int
          frameManagerLock.Lock()
          

          -- step 1: wait on newFramesAvailable until enough free frames to satisfy the request
          while numberFreeFrames < numFramesNeeded
            newFramesAvailable.Wait(&frameManagerLock)
          endWhile

          -- step 2: do a for loop for each frames
          for i = 0 to numFramesNeeded-1

            -- step 3: determine which frame is free 
            frame = framesInUse.FindZeroAndSet()

            -- step 4: figure out the address of the free frames
            frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (frame * PAGE_SIZE)

            -- step 5: execute aPageTable.SetFrameAddr(i,frameAddr)
            aPageTable.SetFrameAddr(i, frameAddr)
          endFor

          -- step 6: Adjust the number of free frames
          numberFreeFrames = numberFreeFrames - numFramesNeeded

          -- step 7: set aPageTable.numberOfPages(numFramesNeeded)
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
          ...
          -- look up the size of page table and iterate through for each frame
          var 
            framesReturned, frameAddr, bitNum, i: int

          -- Step 0: lock the mutex because monitor style
          frameManagerLock.Lock()

          -- Step 1: look at the number of pages -> frames returned
          framesReturned = aPageTable.numberOfPages

          -- Step 2: iterate from 0 -> framesReturned
          for i = 0 to framesReturned-1
            -- Step 3: extract frame address of each frame
            frameAddr = aPageTable.ExtractFrameAddr(i)

            -- Step 4: find out the bit number
          ...
          -- step X: replace the number of frames back and broadcast
          numberFreeFrames = numberFreeFrames + framesReturned
...
-----------------------------  DiskInterruptHandler  --------------------------
  ...
  function DiskInterruptHandler ()
    ...
    --
      ...
      -- Uncommented the following block based on p5 code setup instructions ----
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  function Handle_Sys_Shutdown ()
      FatalError("\nA system shutdown was raised by a user thread\n")
    endFunction

-----------------------------  Handle_Sys_Yield  ---------------------------------

  function Handle_Sys_Yield ()
      print("\nYield invoked by some user thread\n")
    endFunction
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
      ...
      print("\nSystem Fork invoked by user thread")
      nl()
      return 1000
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
      print("System Join invoked by a user thread with process ID:")
      printInt(processID)
      nl()
      return 2000
    endFunction
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      var 
        ...
        initUserStackTop, initPC, i, capture_file_error: int
        initSystemStackTop: ptr to int
        fName: array [MAX_STRING_SIZE] of char

        capture_file_error = currentThread.myProcess.addrSpace.GetStringFromVirtual(&fName, filename asInteger, MAX_STRING_SIZE)

        if capture_file_error < 0
          FatalError("Could not read file from user thread")
        endIf
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      ...
      var 
        file: array [MAX_STRING_SIZE] of char
        err: int
      err = currentThread.myProcess.addrSpace.GetStringFromVirtual(&file, filename asInteger, MAX_STRING_SIZE)

      if err < 0
        return -1
      endIf
      print("\nSystem Create invoked by the user thread")
      nl()

      print("File created name: ")
      print(&file)

      print("File virtual address: ")
...
-----------------------------  Handle_Sys_Open  ---------------------------------
  ...
  function Handle_Sys_Open (filename: ptr to array of char) returns int
      ...
      print("System Open was invoked by a User thread")
      nl()
...
-----------------------------  Handle_Sys_Write  ---------------------------------
  ...
  function Handle_Sys_Write (fileDesc: int, buffer: ptr to char, sizeInBytes: int) returns int
      ...
      print ("System Write was invoked")
      nl()
...
-----------------------------  Handle_Sys_Close  ---------------------------------
  ...
  function Handle_Sys_Close (fileDesc: int)
      print("System Close was invoked by a user thread")
      nl()
      print("File description:")
      printInt(fileDesc)
      nl()
...
---------------------- Code inserted from FileStuff.c in p5 ----------------------
...
--------------------------  InitFirstProcess  -------------------------------
--
--  This function is a wrapper to the StartFirstProcess() 
--  Spawns the thread that will execute the user level process. 
--  Only Inits and forks a thread 
...
function InitFirstProcess()
  ...
  th.Init("MyProgramRunner")
  th.Fork(StartFirstProcess, 0)

endFunction

--------------------------  StartFirstProcess  -------------------------------
--
--  This function loads the user level process from the disk and executes it.
--  It does this by allocating new a address space in the form of PCBs
--  and then uses File IO to open the file loaded using diskUtil from the starting sector
--  It also makes this user process non-privileged by enabling interrupts in BecomeUserThread() method

function StartFirstProcess(something: int)

  var 
    pcb: ptr to ProcessControlBlock
    tempOpen: ptr to OpenFile
    initPC: int
    initUserStackTop: int
    initSystemStackTop: ptr to int
    interrupts: int

  -- Allocating new PCB to a process
  pcb = processManager.GetANewProcess()

  -- Linkning the PCB to the thread in control
  pcb.myThread = currentThread

  -- Reverse linking thread''s process to the PCB
  currentThread.myProcess = pcb


  -- Open a new file using the file manager for maintaining resources
  tempOpen = fileManager.Open("TestProgram2")
  if !tempOpen
    FatalError("file open failed")
  endIf

  -- Loading the executable file using the file Descriptors
  initPC = tempOpen.LoadExecutable(&pcb.addrSpace)

  -- Initializing User Stack top as we will have to switch this to user level process from system level
  initUserStackTop = (pcb.addrSpace.numberOfPages * PAGE_SIZE)

  -- Initializing System stack 
  initSystemStackTop = &currentThread.systemStack[SYSTEM_STACK_SIZE-1]

  -- Memory management - close the fileDesc
  fileManager.Close(tempOpen)

  -- Making this user thread privileges --
  interrupts = SetInterruptsTo (DISABLED)
  pcb.addrSpace.SetToThisPageTable ()
  currentThread.isUserThread = true

  -- Call the BecomeUserThread to finish switching --
  BecomeUserThread (initUserStackTop, initPC, initSystemStackTop asInteger)
